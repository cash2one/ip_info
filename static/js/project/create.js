...
$(document).ready(function(){
	...
	function setContent(types, dep, remo) {
        ...
        var department = ''
        if(dep==3){ department='site'}
        else if (dep==4) {department='ipad'}
        else if (dep==5) {department='tw'}
        else{department='api'}
        var remote = ''
        remote = remo    
        var params = { 
        ...
        $.getJSON(postUrl, params, function(items) {
            ...
            $related.on('change', function(x) {
                ...
                if (current.val() > 0) {
                    ...
                    $branch.text(current.attr('data-type') + '-' + current.val() + ''+ '-' +current.attr('data-department')).show().next('input').attr('placeholder', '-desc(可不填)')
    ...
    $('.types').on('change', function(x,y) {
		if (!$(this).is(':checked')) {
			...
			}else if($('#department_2').is(':checked')){
                                dep = $('#department_2').val();
                        }else if($('#department_3').is(':checked')){
                                dep = $('#department_3').val();
                        }else {
				dep = $('#department_4').val();
			};
            var remote='';
			$(".remote").each(function(){
			if($(this).attr("checked")) 
